...
<div class="container-fluid py-4">
    ...
    <!-- ROW 4: Weather, Top Risk, AI Recommendation -->
    <div class="row g-4">
        <!-- Weather Trend -->
        <div class="col-xl-4">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold mb-0 text-dark">{{ __('Weather Trend (Global Avg)') }} <i class="bi bi-info-circle text-muted ms-1" style="font-size: 0.8rem;"></i></h6>
                </div>
                <div class="card-body p-3">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <span style="font-size:1.2rem;">
                                                ...
                                                @endphp
                                                <img src="https://flagcdn.com/24x18/{{ $code }}.png" alt="{{ $code }}">
                                            </span>
                                            <span class="fw-semibold" style="font-size:0.85rem;">{{ $rc->country->name ?? 'Unknown' }}</span>
                                        </div>
                                    </td>
                                    <td>
                                        @php
                                        ...
                                        @endphp
                                        <span class="badge bg-{{ $bColor }}" style="{{ $bColorStyle }}">{{ __($level) }}</span>
                                    </td>
                                    <td class="fw-bold">{{ $rc->final_score ?? 0 }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
        ...
        <div class="col-xl-4">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold mb-0 text-dark">{{ __('AI Recommendation') }} <i class="bi bi-info-circle text-muted ms-1" style="font-size: 0.8rem;"></i></h6>
                </div>
                <div class="card-body p-4 d-flex flex-column">
                    @if(!empty($aiRecommendation))
                    <div class="d-flex align-items-center gap-3 mb-4">
                        <div class="rounded-3 bg-success bg-opacity-10 d-flex align-items-center justify-content-center text-success" style="width: 60px; height: 60px;">
                            <i class="bi bi-robot fs-2"></i>
                        </div>
                        <div>
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <span class="fw-bold text-dark" style="font-size:0.85rem;">{{ __('Overall Sentiment') }}</span>
                                <span class="badge bg-warning text-dark bg-opacity-25" id="ai-sentiment">{{ $aiRecommendation['overall_sentiment'] ?? 'Neutral' }}</span>
                            </div>
                            <div class="d-flex align-items-center gap-2">
                                <span class="fw-bold text-dark" style="font-size:0.85rem;">{{ __('Confidence Score') }}</span>
                                <span class="fw-bold text-success" id="ai-confidence">{{ $aiRecommendation['confidence_score'] ?? '0' }}%</span>
                            </div>
                        </div>
                    </div>
                    
                    <p class="text-muted" style="font-size:0.85rem; line-height: 1.6;" id="ai-message">
                        {{ $aiRecommendation['recommendation'] ?? 'No recommendation available.' }}
                    </p>
                    
                    <div class="mt-auto pt-3 text-center">
                        <a href="{{ route('user.news') }}" class="btn btn-outline-primary rounded-pill px-4 btn-sm fw-semibold">{{ __('View Full Analysis') }} <i class="bi bi-arrow-right ms-1"></i></a>
                    </div>
                    @else
                    <div class="text-center py-5">
                        <i class="bi bi-robot text-muted fs-2 mb-3 d-block"></i>
                        <p class="text-muted">No AI Recommendation available yet.</p>
                    </div>
                    @endif
                </div>
            </div>
